...
class Employees extends CI_Controller {
  ...
  public function reset_password($id){
    ...
    $this->form_validation->set_rules('form_id','banner id', 'trim|required|is_unique[users.banner_id]|numeric');
    $this->form_validation->set_rules('form_password', 'Password', 'trim|required|matches[form_password]');
    $this->form_validation->set_rules('form_re-enter_password', 'Password', 'trim|required|matches[form_password]');
    // $this->form_validation->set_rules('form_confirm_password', 'Password Comformation', 'trim|required|matches[form_password]');

    //Form info is invalid
    if($this->form_validation->run() == false){
      //TODO: Reload the page and tell them passowrds dont match
    }else{
      if($this->input->post('form_password') == $this->input->post('form_re-enter_password')){
        //set data
        $data = array(
          'banner_id' => $this->input->post('form_id'),
          'password' => password_hash($this->input->post('form_password'), PASSWORD_BCRYPT)
        ...
        //insert into db
        if($this->User_Model->password_reset($data)){
          $this->session->set_flashdata('message', '<div class="alert alert-success text-center">Successfully Changed Password. </div>');
        } else {
          $this->session->set_flashdata('message', '<div class="alert alert-danger text-center">Error. Please try again. </div>');
        }
      }else{
        $this->session->set_flashdata('message', '<div class="alert alert-danger text-center">Passwords do not match. </div>');
